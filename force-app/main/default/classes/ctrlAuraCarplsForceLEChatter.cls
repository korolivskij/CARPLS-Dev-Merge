...
global without sharing class ctrlAuraCarplsForceLEChatter {
    ...
    */
    ...
    global static List<CarplsFeedItem> retrieveQuestions(List<Id> lParentIds, Id caseId) {
        ...
        List<CarplsFeedItem> cfis = retrieveQuestions(sParentIds,caseId);
    ...
    */
    ...
    public static ResponseDTO createFeedItem(FeedItem fi, Id caseId, Network network) {
        ...
        if (fi == null) {
            dto.message+= 'FeedItem passed is null we cant do anything with null';
            return dto;
        }
        system.debug('--- create feedItem with network:'+network);
        ...
        if(pId == null) {
            dto.message+= 'FeedItem passed has a null parentId must set parent!';
            return dto;
        }
        if(pId.contains('0F9')) {
            //fi.NetworkScope = 'AllNetworks'; //@TODO double check this! do we need this?
        ...
        try {
            ...
            system.debug('--- attempting fi insert. fi='+fi);
            insert fi;
